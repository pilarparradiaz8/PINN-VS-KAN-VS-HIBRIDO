...
class Hybrid(nn.Module):
    ...
    def psi(self,x,t):
        xt = torch.cat([x,t],1)
        psi = self.w_kan*self.kan_psi(xt) + self.w_pinn*self.pinn_psi(xt)

        return psi

    def u(self,t):
        return self.w_kan*self.kan_u(t) + self.w_pinn*self.pinn_u(t)
...
t_f = torch.rand(3000,1).to(device)*T
...
# =========================================================
# 5. GRÁFICAS DEL LÁSER Y DEL SALTO (ADAPTADO)
# =========================================================
...
with torch.no_grad():
    ...
    psi_final  = model.psi(x_plot, torch.ones_like(x_plot) * T_final)
...
axs[2].plot(x_plot.cpu().numpy(), prob_final.cpu().numpy(), color='green', label=f"Hybrid (t={T_final})")
...
plt.plot(t_plot_cpu, normas_totales, color='black', linewidth=2.5)
...
plt.figure(figsize=(8, 4))
plt.plot(t_plot_cpu, doble_integral_acumulada, color='orange', linewidth=2.5)
...
# =========================================================
# 8. GIF FINAL (ESTILO PRO)
# =========================================================

print("Preparando GIF...")

num_frames = 400
x_numpy = x_plot.cpu().numpy().flatten()
t_grid = torch.linspace(0, T_final, num_frames).to(device)
...
ani.save("salto_hibrido.gif", writer='pillow', fps=40)

plt.close()
print("GIF guardado como salto_hibrido.gif")
