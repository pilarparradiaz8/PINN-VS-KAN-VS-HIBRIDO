#CODIGO KAN POLYNOMIAL (2 CAPAS Y 32 NEURONAS)
...
class Capa_Polynomial_KAN(nn.Module):
    def __init__(self, in_dim, out_dim, grado=4):
        super().__init__()
        self.grado = grado

        # Coeficientes polinómicos
        self.coef_poly = nn.Parameter(
            torch.randn(out_dim, in_dim, grado) * 0.02
        )
    ...
    def forward(self, x):
        ...
        # IMPORTANTE: acotar entrada para estabilidad numérica
        x_bound = torch.tanh(x)

        # Construcción polinómica: Base de Chebyshev
        T0 = torch.ones_like(x_bound)
        T1 = x_bound

        # k = 1
        salida += torch.einsum('oi,bi->bo', self.coef_poly[:, :, 0], T1)

        # k >= 2
        for k in range(2, self.grado + 1):
            Tk = 2 * x_bound * T1 - T0
            salida += torch.einsum('oi,bi->bo', self.coef_poly[:, :, k-1], Tk)
            T0, T1 = T1, Tk

        return salida

class KAN_Net(nn.Module):
    def __init__(self, layers, grado=4):
        super().__init__()
        self.capas = nn.ModuleList([
            Capa_Polynomial_KAN(layers[i], layers[i+1], grado)
...
class KAN_Control_Cuantico:
    def __init__(self):
        capas_psi = [2, 32, 32, 2]
        self.net_psi = KAN_Net(capas_psi, grado=4).to(device)

        capas_control = [1, 16, 16, 1]
        self.net_control = KAN_Net(capas_control, grado=4).to(device)
