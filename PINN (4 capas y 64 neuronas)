...
plt.close(fig_psi)
# =========================================================
# 9. ANÁLISIS TERMODINÁMICO CORREGIDO (W = ΔE)
# =========================================================
print("\nVerificando la Termodinámica (forma continua con autograd)...")
...
# 🔥 GRID TEMPORAL
t_vals = np.linspace(0, T_final, 400)
dt_calc = t_vals[1] - t_vals[0]

# ⚠️ LONGITUD SEGURA
N = min(len(t_vals), len(P1), len(P2), len(P3), len(P4), len(P5))
...
# 🔥 Tensor temporal
t_tensor = torch.tensor(t_vals, dtype=torch.float32, requires_grad=True).view(-1,1).to(device)

u = modelo_control.net_control(t_tensor)
...
du_dt_np = du_dt.detach().cpu().numpy().flatten()
...
with torch.no_grad():
    for i in range(N):
        ...
        # -------- ψ --------
        t_tensor_local = torch.ones_like(x_plot) * t_val
        psi = modelo_control.net_psi(torch.cat([x_plot, t_tensor_local], dim=1))
        ...
        # -------- TRABAJO --------
        trabajo_total += centro_x * du_dt_np[i] * dt_calc
...
# =========================================================
# GRÁFICA FINAL W vs ΔE
# =========================================================
plt.figure(figsize=(10, 6))

plt.plot(t_vals, trabajo_acumulado,
         color='cyan', lw=4, label='Trabajo del Láser ($W$)')

plt.plot(t_vals, aumento_energia_interna,
         color='magenta', lw=2, linestyle='--',
         label='Aumento Energía ($\\Delta E$)')

plt.axhline(y=salto_teorico,
            color='red', linestyle=':',
            label=f'Meta Teórica ({salto_teorico:.2f})')

plt.title("Primera Ley de la Termodinámica: $W = \\Delta E$")
plt.xlabel("Tiempo (t)")
plt.ylabel("Energía")
